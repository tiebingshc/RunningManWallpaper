...
import com.leo.runningman.util.DiskLruCache;
...
public class ImageGridFragment extends Fragment implements AdapterView.OnItemClickListener {
    ...
    private int start=0;				//the start position to require data
    private int count = 90;					//dynamic add data num
    /**
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mImageWorker.setAdapter(ImageProvider.imageThumbWorkerUrlsAdapter);
	...
	public void autoHideview(long millions)
	{
		if(hideThread == null){
			hideThread = new Thread(){
				public void run(){
					sendHandlerMessage(HIDE_BOTTOM, 0, 0, null);
				}
			};
		}
		...
		{
			cancelAutoHideview();
			layout_bottom.postDelayed(hideThread, millions);
		}
	}
	
	public void autoShowview(long millions)
	{
		if(showThread == null){
			showThread = new Thread(){
				public void run(){
					sendHandlerMessage(SHOW_BOTTOM, 0, 0, null);
				}
			};
		}
		if(layout_bottom.getVisibility() != View.VISIBLE)
		{
			cancelAutoShow();
			layout_bottom.postDelayed(showThread, millions);
		}
	}
    
	public void cancelAutoShow(){
		if(showThread != null){
			layout_bottom.removeCallbacks(showThread);
		}
	}
	
	public void cancelAutoHideview(){
		if(hideThread != null){
			layout_bottom.removeCallbacks(hideThread);
		}
	}
	...
	private static final int HIDE_BOTTOM = 1;
	...
	private void showBottomLayout() {
		if(layout_bottom != null && layout_bottom.getVisibility() != View.VISIBLE){
	...
	private void hideBottomLayout() {
		if(layout_bottom != null && layout_bottom.getVisibility() != View.GONE){
	...
	class HttpGetTask extends AsyncTask<String, Void, Void>{
		...
		protected Void doInBackground(String... params) {
			Network.getInstance(getActivity()).getUrls(1,start,count);
			return null;
		}
    ...
    public void onResume() {
        ...
        if(mAdapter != null && mAdapter.getCount()>3){			//TODO  numColum:3
        		mAdapter.notifyDataSetChanged();
        }else if(mAdapter.getCount()<3){
        		new HttpGetTask(1000L).execute();
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.clear_cache:
                ...
                return true;
//            case R.id.local:
//            	Intent intent = new Intent(this.getActivity(),LocalImageActivity.class);
